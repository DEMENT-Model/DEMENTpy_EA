...
# def daily_substrate_input(subst, itime, parameters):
#     """
#     Provides periodic substrate input with sekectable litter composition
#     """
...
#                 scal_vec[()] = np.array(_litter_compo[litter_keys[istep]])
#                 break
#         scal_vec = scal_vec / 365.0
#         return scal_vec

#     if (not hasattr(daily_substrate_input, 'buff_dat')):
#         bdat = {}
#         base_stoech = np.zeros(subst.mass.shape[2:])
#         for ksub,d in _substrates_default_stoech.items():
#             isub=  subst.get_component_index(ksub)
...
#             'scal_vec':scal_vec
#             }
#         daily_substrate_input.buff_dat = bdat

#     bdat = daily_substrate_input.buff_dat
#     bdat['scal_vec'] = set_rescaling_vector(itime, bdat['scal_vec'])
#     tmp_base = bdat['base_stoech'] * bdat['scal_vec'][:,np.newaxis]
#     bdat['res_buff'][()] = tmp_base[np.newaxis,np.newaxis,:,:]
#     daily_substrate_input.buff_dat = bdat
#     del(tmp_base)
#     return daily_substrate_input.buff_dat['res_buff']

def substrate_input(subst, itime, parameters):
    """
    Provides periodic substrate input with sekectable litter composition
    """
    ...
    def set_rescaling_vector(itime,scal_vec):
        ...
        for istep,t in enumerate(stvals):
            if (i_year_int <= t):
                scal_vec[()] = np.array(_litter_compo[litter_keys[istep]])
                break
        scal_fact = {'Yearly':1.0,'Daily':1.0/365.0}[src_type]
        scal_vec = scal_vec * scal_fact
        return scal_vec

    if (not hasattr(substrate_input, 'buff_dat')):
        bdat = {}
        base_stoech = np.zeros(subst.mass.shape[2:])
        for ksub,d in _substrates_default_stoech.items():
            isub=  subst.get_component_index(ksub)
        ...
        substrate_input.buff_dat = bdat

    is_active = {
            'Yearly':((itime >= 0) and ((itime % 365) == 0)),
            'Daily': True,
            }[src_type]
    if is_active:
        bdat = substrate_input.buff_dat
        bdat['scal_vec'] = set_rescaling_vector(itime, bdat['scal_vec'])
        tmp_base = bdat['base_stoech'] * bdat['scal_vec'][:,np.newaxis]
        bdat['res_buff'][()] = tmp_base[np.newaxis,np.newaxis,:,:]
        substrate_input.buff_dat = bdat
        del(tmp_base)
        return substrate_input.buff_dat['res_buff']
    else:
        return 0
